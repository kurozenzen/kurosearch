...
describe("pages", () => {
  ...
  describe("getPage", () => {
    test("invalid page number throws TypeError", () => {
      expect.assertions(1);
      return getPage(undefined, [], "id", 0).catch((e) =>
    ...
    test("invalid tag array throws TypeError", () => {
      expect.assertions(1);
      return getPage(0, undefined, "id", 0).catch((e) =>
        expect(e).toBeInstanceOf(TypeError)
      );
    });
    ...
    test("invalid sort returns TypeError", () => {
      expect.assertions(1);
      return getPage(0, [], undefined, 0).catch((e) =>
        expect(e).toBeInstanceOf(TypeError)
      );
    });
    ...
    test("invalid minScore TypeError", () => {
      expect.assertions(1);
      return getPage(0, [], "id", undefined).catch((e) =>
        expect(e).toBeInstanceOf(TypeError)
      );
    });
  });
  ...
  describe("getPostsUrl", () => {
    test("does not include tags when they are empty", () => {
      expect(getPostsUrl(0, "")).toBe(
        "https://r34-json.herokuapp.com/v2/posts?limit=20&pid=0"
      );
    });

    test("includes tags when they are not empty", () => {
      expect(getPostsUrl(0, "example")).toBe(
        "https://r34-json.herokuapp.com/v2/posts?limit=20&pid=0&tags=example"
      );
    });
  });
  ...
  describe("serializeTags", () => {
    ...
    test("+ tags are included in string", () => {
      expect(
        serializeTags(
          [new SearchableTag("+", "tag1"), new SearchableTag("+", "tag2")],
          "id",
          0
        )
      ).toBe("tag1+tag2+sort:id:desc+score:>=0");
    });
    ...
    test("~ tags are included in string", () => {
      expect(
        serializeTags(
          [new SearchableTag("~", "tag2"), new SearchableTag("~", "tag3")],
          "id",
          0
        )
      ).toBe("sort:id:desc+score:>=0+( tag2 ~ tag3 )");
    });
  });
});
